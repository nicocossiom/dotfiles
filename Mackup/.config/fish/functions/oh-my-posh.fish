set --export POSH_THEME /Users/pepperonico/.config/fish/themes/gruvbox_nico.json
...
function fish_prompt
    ...
    if test "$omp_transient" = "1"
      /opt/homebrew/bin/oh-my-posh print transient --config $POSH_THEME --shell fish --error $omp_status_cache --execution-time $omp_duration --stack-count $omp_stack_count --shell-version $FISH_VERSION --no-exit-code=$omp_no_exit_code
      return
    end
    ...
    /opt/homebrew/bin/oh-my-posh print primary --config $POSH_THEME --shell fish --error $omp_status_cache --execution-time $omp_duration --stack-count $omp_stack_count --shell-version $FISH_VERSION --cleared=$omp_cleared --no-exit-code=$omp_no_exit_code
end
